...
const DoGuideReview = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const review = {
      ...
      guideName: bookings.gname,
      guidePhoto: bookings.gphoto,

    };

    try {
      const response = await fetch('https://outingbd-server.vercel.app/guidereviews', {
        method: 'POST',
        headers: {
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            ...
            <div className=' grid justify-center space-y-2 text-center'>
              <h2 className='text-xl font-bold'>Rating for:{bookings.gname}</h2>



              <img className='h-52 w-72 rounded-lg' src={bookings.gphoto} alt="" />
            </div>
